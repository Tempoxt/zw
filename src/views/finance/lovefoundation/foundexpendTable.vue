...
const api_resource = api_common.resource("lovefoundation/foundexpend");
const defaultForm = () => {
    return {
        estate:1,
        roomAdmin:''
...
export default {
  ...
  methods: {
    ...
    async allocation(){
        ...
        await this.$request.put('/lovefoundation/foundgrant/'+row.id)
        this.fetchTableData()
  ...
  async created() {
    const { field, action,table } = await api_common.menuInit("lovefoundation/foundexpend");
