              ...
              >
                <template v-if="!data.roomName">
                  ...
                  <span style="padding-right:10px"
                    >({{ data.totalBeds }} - {{ data.usedBeds }})</span
                  >
                </template>
                <template v-else>
                  <div>
                    <span
                      class="icon iconfont icon-nv"
                      v-if="data.dormType === 0"
                      style="color:rgba(255, 52, 160, 1)"
                    ></span>
                    <span
                      class="icon iconfont icon-nan"
                      v-if="data.dormType === 1"
                      style="color:rgba(11, 178, 212, 1)"
                    ></span>
                    &nbsp;
...
export default {
  ...
  methods: {
    filterNode(value, data) {
      ...
      return (
        (data.dormName && data.dormName.indexOf(value) !== -1) ||
        (data.roomName && data.roomName.indexOf(value) !== -1)
      );
    },
    ...
    async getTree() {
      this.data2 = [
        await api_common.resource("dormitory/dormtree/backend").get()
      ];
      this.$nextTick(() => {
        this.$refs.tree2.setCurrentKey(this.orgid);
      });
