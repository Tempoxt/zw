               ...
               <el-form ref="form" :model="roleForm" label-width="80px" label-position="left">
                <el-row :gutter="20">
                   <el-col :span="12">
                    ...
                    <form-render :type="`select`" :field="{name:'所在分类',options:data2.map(item=>({label:item.groupname,value:item.id}))}" v-model="roleForm.groupid"/>
                  </el-col>
                  <el-col :span="12">
        ...
        <div>
          ...
          >
            <span slot-scope="{ node, data }" class="node-actions">
              ...
              <div class="actions">
                <span class="icon iconfont icon-lajitong" @click="(node.isLeaf&&data.name)?deleteRole(node,data,...arguments):deleteCategory(node,data,...arguments)"></span>
                <span class="icon iconfont icon-bianji"  @click="(node.isLeaf&&data.name)?updateRole(node,data,...arguments):updateCategory(node,data,...arguments)"></span>
              </div>
...
export default {
  ...
  methods: {
    ...
    async deleteCategory(node,data,e){
      e.stopPropagation()
      await this.$confirm('正在删除'+data.groupname+'分类, 是否继续?', '提示', {
